...
def format_observation(observation: Dict[str, Any], *, use_color: bool=False) -> str:
    ...
    lines = [c('--- Eurika Scan Report ---', _CYAN), f"Files: {summary.get('files', 0)}", f"Lines: {summary.get('total_lines', 0)}", f"Smells: {(c(str(summary.get('smells_count', 0)), _YELLOW) if summary.get('smells_count') else '0')}", f"Duplicate blocks: {summary.get('duplicates_count', 0)}", '']
    if structure:
...
def format_observation_md(observation: Dict[str, Any]) -> str:
    ...
    structure = observation.get('structure', [])
    duplicates = observation.get('duplicates', [])
    lines = ['# Eurika Scan Report', '', f"- **Files:** {summary.get('files', 0)}", f"- **Lines:** {summary.get('total_lines', 0)}", f"- **Smells:** {summary.get('smells_count', 0)}", f"- **Duplicate blocks:** {summary.get('duplicates_count', 0)}", '']
    if structure:
...
def health_summary_enhanced(health: Dict[str, Any], *, use_color: bool=False, ascii_chart: bool=True) -> str:
