...
def _get_git_commit(project_root: Path) -> Optional[str]:
    ...
    try:
        ...
        if r.returncode == 0 and r.stdout:
            return r.stdout.strip()[:12]  # short hash
...
class ArchitectureHistory:
            ...
            for item in raw.get("history", []):
                self._points.append(
                    HistoryPoint(
                        ...
                        modules=item.get("modules", 0),
                        dependencies=item.get("dependencies", 0),
                        cycles=item.get("cycles", 0),
                        max_degree=item.get("max_degree", 0),
                        total_smells=item.get("total_smells", 0),
                        smell_counts=item.get("smell_counts", {}),
    ...
    def _save(self) -> None:
        data = {
            "history": [asdict(p) for p in self._points],
        }
        try:
    ...
    def append(self, graph: ProjectGraph, smells: List[ArchSmell], summary: Dict) -> None:
        ...
        # v0.6: version, git_commit, risk_score
        ...
        point = HistoryPoint(
            timestamp=time.time(),
            modules=int(sys.get("modules", g_sum.get("nodes", 0))),
            dependencies=int(sys.get("dependencies", g_sum.get("edges", 0))),
            cycles=int(sys.get("cycles", len(g_sum.get("cycles", [])))),
    ...
    def trend(self, window: int = 5) -> Dict[str, str]:
        ...
        if len(pts) < 2:
            return {
                ...
                "smells": "insufficient_data",
                "centralization": "insufficient_data",
            }
    ...
    def detect_regressions(self, window: int = 5) -> List[str]:
        ...
        # v0.6: track god_module, bottleneck, hub separately
        ...
        if newest.max_degree > oldest.max_degree * 1.5 and oldest.max_degree >= 2:
            notes.append(
                f"Centralization increased significantly (max degree {oldest.max_degree} → {newest.max_degree})"
            )
    ...
    def _append_maturity_section(self, lines: List[str], maturity: str) -> None:
        ...
        if maturity == "high":
            lines.append(
                "Interpretation: architecture is growing or stabilizing with decreasing structural issues. "
                "Focus can shift from firefighting to guided evolution."
            )
        elif maturity == "medium":
            lines.append(
                "Interpretation: architecture is stable but still centralized. "
                "Monitor hubs and bottlenecks to avoid future rigidity."
            )
        elif maturity == "low":
            lines.append(
                "Interpretation: structural issues are accumulating. "
                "Prioritize breaking emerging bottlenecks and reducing smell growth."
            )
    ...
    def evolution_report(self, window: int = 5, ascii_chart: bool = True) -> str:
        ...
        # v0.6: snapshot meta (version, git_commit, risk_score)
        ...
        # v0.8: diff metrics (deltas) over the window
        if len(pts) >= 2:
            lines.append("Diff metrics (window):")
            lines.append(
                f"- Modules: {oldest.modules} → {newest.modules} "
                f"(Δ {newest.modules - oldest.modules})"
            )
            lines.append(
                f"- Dependencies: {oldest.dependencies} → {newest.dependencies} "
                f"(Δ {newest.dependencies - oldest.dependencies})"
            )
            lines.append(
                f"- Cycles: {oldest.cycles} → {newest.cycles} "
                f"(Δ {newest.cycles - oldest.cycles})"
            )
            lines.append(
                f"- Total smells: {oldest.total_smells} → {newest.total_smells} "
                f"(Δ {newest.total_smells - oldest.total_smells})"
            )
            lines.append(
                f"- Max degree: {oldest.max_degree} → {newest.max_degree} "
                f"(Δ {newest.max_degree - oldest.max_degree})"
            )
            if oldest.risk_score is not None and newest.risk_score is not None:
                lines.append(
                    f"- Risk score: {oldest.risk_score} → {newest.risk_score} "
                    f"(Δ {newest.risk_score - oldest.risk_score})"
                )
            lines.append("")

            # Smell history (per-type counts over the window)
