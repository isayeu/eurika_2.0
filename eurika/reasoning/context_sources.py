...
def _extracted_block_135(k: str, rejected_targets: list[str]) -> None:
...
def build_context_sources(project_root: Path, operations: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for k in rejected_keys:
        _extracted_block_135(k, rejected_targets)
    ...
    for op in operations:
        ...
        if target in by_target:
            continue
        entry: dict[str, Any] = {'related_tests': _related_tests(root, target), 'neighbor_modules': _neighbor_modules(root, target)}
    ...
    try:
        ...
        pass
    return {'recent_verify_fail_targets': verify_fail_targets[:10], 'campaign_rejected_targets': rejected_targets[:10], 'recent_patch_modified': _recent_patch_modified(root, limit=10), 'by_target': by_target, 'prioritized_smell_actions': prioritized_smell_actions}