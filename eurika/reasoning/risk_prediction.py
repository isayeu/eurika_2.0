...
def _extracted_block_41(node, s, scores: Dict, reasons: Dict) -> None:
    sev = float(getattr(s, 'severity', 0) or 0)
    scores[node] = scores.get(node, 0.0) + sev * 2.5
    reasons.setdefault(node, []).append(s.type)

def predict_module_regression_risk(project_root: Path, top_n: int=10) -> List[Dict[str, Any]]:
    """
    ...
    for s in smells:
        for node in s.nodes:
            _extracted_block_41(node, s, scores, reasons)
    ...
    for module, raw in ordered:
        ...
        result.append({'module': module, 'score': score, 'reasons': list(dict.fromkeys(reasons.get(module, [])))})
    return result