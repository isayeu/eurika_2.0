...
def _extract_code_smell_patterns(project_root: Path, project: str) -> dict[str, list[dict[str, Any]]]:
    ...
    try:
        ...
        for file_path in list(analyzer.scan_python_files())[:100]:
            try:
                ...
                for smell in analyzer.find_smells(file_path):
                    ...
                    entry = {'project': project, 'module': rel, 'location': loc, 'severity': getattr(smell, 'metric', 0) or 0, 'hint': hint}
...
def _extracted_block_119(node, project, s, hint, patterns):
    entry = {'project': project, 'module': node, 'severity': round(s.severity, 2), 'hint': hint}
...
def extract_patterns_from_repos(cache_dir: Path) -> dict[str, Any]:
    ...
    """
    patterns: dict[str, list[dict[str, Any]]] = {'god_module': [], 'hub': [], 'bottleneck': [], 'cyclic_dependency': [], 'long_function': [], 'deep_nesting': []}
            ...
            try:
                ...
                for s in smells:
                    ...
                    for node in (s.nodes or [])[:3]:
                        _extracted_block_119(node, project, s, hint, patterns)
