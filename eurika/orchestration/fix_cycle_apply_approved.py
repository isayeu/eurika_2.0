...
def _announce_apply_to_chat(path: Path, *, exit_code: int) -> None:
    """Surface apply-approved outcome in Chat / Goals (Qt + Desktop poll)."""
    try:
        from eurika.api.fix_status import announce_apply_approved

        announce_apply_approved(
            path,
            exit_code=exit_code,
            client="fix_cli",
            # RPC/Desktop already mirrors approval/apply; chat.jsonl is enough for Qt.
...
def run_apply_approved_path(path: Path, *, session_id: str | None, quiet: bool, verify_cmd: str | None, verify_timeout: int | None, run_params: dict[str, Any] | None = None, deps: FixCycleDeps, execute_fix_apply_stage: Callable[..., tuple[FixReport, list[str], bool]], build_fix_cycle_result: Callable[[FixReport, list[OperationRecord], list[str], bool, Any], dict[str, Any]], attach_fix_telemetry: Callable[[FixReport, list[OperationRecord]], None]) -> dict[str, Any]:
    ...
    if not payload:
        ...
        out = with_cycle_state({'return_code': 1, 'report': rep, 'operations': [], 'modified': [], 'verify_success': False, 'agent_result': None}, is_error=True)
    ...
    if not approved:
        ...
        out = with_cycle_state({'return_code': 0, 'report': report, 'operations': [], 'modified': [], 'verify_success': True, 'agent_result': None}, is_error=False)
        _announce_apply_to_chat(path, exit_code=0)
        return out
