...
def build_fix_dry_run_result(path: Path, patch_plan: PatchPlan, operations: list[OperationRecord], result: Any, *, run_params: dict[str, Any] | None=None) -> dict[str, Any]:
    ...
    try:
        ...
        pass
    return with_cycle_state({'return_code': 0, 'report': report, 'operations': operations, 'modified': [], 'verify_success': None, 'agent_result': result, 'dry_run': True}, is_error=False)
...
def _update_execution_context_after_rescan(result: Any, report: FixReport, path: Path, build_snapshot_from_self_map: Any) -> None:
...
def enrich_report_with_rescan(path: Path, report: FixReport, rescan_before: Path, quiet: bool, run_scan: Any, build_snapshot_from_self_map: Any, diff_architecture_snapshots: Any, metrics_from_graph: Any, rollback_patch: Any) -> None:
...
def _extracted_block_208(report):
    return report.get('aborted_reason') or (report.get('rollback') or {}).get('reason') or 'verify_failed'

def append_fix_cycle_memory(path: Path, result: Any, operations: list[OperationRecord], report: FixReport, verify_success: Any) -> None:
    ...
    try:
        ...
        modules_for_learning = modified or [str(op.get('target_file')) for op in operations if op.get('target_file')]
        ...
        if operations:
            ...
            if path and env_bool("EURIKA_WEIGHT_ADAPTATION", default=True):
                try:
                    lr = 0.02
                    if env_bool("EURIKA_META_CONTROLLER"):
                    ...
                    pass
        out: dict[str, Any] = {'modified': modified, 'skipped': report.get('skipped', []), 'run_id': report.get('run_id'), 'verify_success': verify_success, 'verify_duration_ms': report.get('verify_duration_ms')}
        delta = report.get('delta_score')
        if delta is not None:
            out['delta_score'] = delta
        if verify_success is False:
            failure_reason = report.get('aborted_reason') or (report.get('rollback') or {}).get('reason') or 'verify_failed'
            out['failure_reason'] = failure_reason
        memory.events.append_event(type='patch', input={'operations_count': len(operations)}, output=out, result=verify_success)
    except Exception:
        pass
...
def attach_fix_telemetry(report: FixReport, operations: list[OperationRecord], path: Path | None=None) -> None:
    ...
    verify_success_raw = verify_payload.get('success') if isinstance(verify_payload, dict) else None
    ...
    telemetry: TelemetryPayload = {'operations_total': ops_total, 'modified_count': len(modified), 'skipped_count': skipped_count, 'apply_rate': round(apply_rate, 4), 'no_op_rate': round(no_op_rate, 4), 'rollback_rate': rollback_rate, 'verify_duration_ms': int(verify_duration_ms)}
    if path is not None:
        median_ms = _median_verify_time_ms(path, int(verify_duration_ms))
        if median_ms is not None:
            telemetry['median_verify_time_ms'] = median_ms
    report['telemetry'] = telemetry
    safety_gates: SafetyGatesPayload = {'verify_required': verify_required, 'auto_rollback_enabled': verify_required, 'verify_ran': verify_ran, 'verify_passed': bool(verify_success_raw) if verify_ran else None, 'rollback_done': rollback_done}
    report['safety_gates'] = safety_gates

def execute_fix_apply_stage(path: Path, patch_plan: PatchPlan, operations: list[OperationRecord], *, session_id: str | None, quiet: bool, verify_cmd: str | None, verify_timeout: int | None, run_params: dict[str, Any] | None=None, backup_dir: str, apply_and_verify: Any, run_scan: Any, build_snapshot_from_self_map: Any, diff_architecture_snapshots: Any, metrics_from_graph: Any, rollback_patch: Any, result: Any) -> tuple[FixReport, list[str], Any]:
    ...
    if simulation.get('errors'):
        ...
        report: FixReport = {'dry_run': False, 'modified': [], 'skipped': simulation.get('would_skip', []), 'skipped_reasons': simulation.get('skipped_reasons') or {}, 'errors': simulation['errors'], 'verify': {'success': False}, 'simulation': simulation, 'risk_report': risk_report.to_dict(), 'aborted_reason': 'simulation_errors'}
        report['operation_explanations'] = [dict(op.get('explainability') or {}, verify_outcome=False) for op in operations]
        report['operation_results'] = [{'target_file': str(op.get('target_file') or ''), 'kind': op.get('kind'), 'approval_state': op.get('approval_state', 'approved'), 'critic_verdict': op.get('critic_verdict', 'allow'), 'applied': False, 'execution_outcome': 'not_applied', 'skipped_reason': 'simulation_errors'} for op in operations]
    ...
    for op in operations:
        ...
        expls.append(expl)
        op_results.append({'target_file': target_file, 'kind': op.get('kind'), 'approval_state': op.get('approval_state', 'approved'), 'critic_verdict': op.get('critic_verdict', 'allow'), 'applied': applied, 'execution_outcome': execution_outcome, 'skipped_reason': skipped_reason or ('not_modified' if not applied else None)})
    ...
    report['llm_hint_runtime'] = result.output.get('llm_hint_runtime')
    if checkpoint_id:
        report['campaign_checkpoint'] = {'checkpoint_id': checkpoint_id, 'reused': bool((checkpoint or {}).get('reused'))}
    attach_fix_telemetry(report, operations, path)
    enrich_report_with_rescan(path, report, rescan_before, quiet, run_scan, build_snapshot_from_self_map, diff_architecture_snapshots, metrics_from_graph, rollback_patch)
    _update_execution_context_after_rescan(result, report, path, build_snapshot_from_self_map)
    if run_params:
        attach_run_params(report, **run_params)
    modified = report.get('modified', [])
    ...
    if checkpoint_id:
        try:
            ...
            cp = attach_run_to_checkpoint(path, checkpoint_id, run_id=report.get('run_id'), verify_success=verify_success, modified=modified)
...
def build_fix_cycle_result(report: FixReport, operations: list[OperationRecord], modified: list[str], verify_success: Any, result: Any) -> dict[str, Any]:
    """Build final run_fix_cycle result payload."""
    return_code = 1 if report.get('errors') or not report['verify'].get('success') else 0
    return with_cycle_state({'return_code': return_code, 'report': report, 'operations': operations, 'modified': modified, 'verify_success': verify_success, 'agent_result': result, 'dry_run': False}, is_error=return_code != 0)