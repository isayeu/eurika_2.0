...
def _extract_phase_from_message(msg: str) -> str | None:
    ...
    msg = (msg or '').strip()
    m = re.search('(?:фазу|phase|фаза)\\s*(CR-[A-Z](?:\\d+)?)', msg, re.I)
    if m:
        return m.group(1).strip().upper()
    m = re.search('(?:фазу|phase|фаза)\\s*([\\d.]+(?:-\\w+)?)', msg, re.I)
    if m:
        return m.group(1).strip()
    m = re.search('(?:реализаци(?:ю|я)|версия|version)\\s*[\\s—-]*(\\d+\\.\\d+(?:\\.\\d+)?(?:-\\w+)?)', msg, re.I)
    if m:
        return m.group(1).strip()
    m = re.search('\\b(CR-[A-Z])(?:\\d+)?\\b', msg, re.I)
    if m:
        return m.group(1).strip().upper()
    m = re.search('\\b(\\d+\\.\\d+(?:\\.\\d+)?(?:-\\w+)?)\\b', msg)
    if m:
        return m.group(1).strip()
    return None
...
def _find_phase_section(content: str, phase: str) -> str | None:
    ...
    if phase.startswith('CR-'):
        ...
        pattern = f'^###\\s+.*[Фф]аза\\s+{phase_escaped}\\b'
        for i, line in enumerate(lines):
            if re.search(pattern, line, re.I):
                start = i
                break
    ...
    while end < len(lines):
        if re.match('^####\\s+', lines[end]) or re.match('^###\\s+', lines[end]) or re.match('^##\\s+', lines[end]):
            break
        end += 1
...
def _parse_phase_table(section: str, phase: str) -> list[dict[str, Any]]:
    ...
    for line in lines:
        ...
        if re.match('^-+$', step_id):
            continue
        is_num_phase = bool(re.match('^[\\d.]+$', step_id))
        is_cr_step = bool(re.match('^CR-[A-Z]\\d+$', step_id, re.I))
        if phase.startswith('CR-'):
            parent = re.match('^(CR-[A-Z])\\d*$', phase, re.I)
            parent_match = parent.group(1).upper() if parent else phase.upper()
            if is_cr_step and (not step_id.upper().startswith(parent_match)):
                continue
            if re.match('^CR-[A-Z]\\d+$', phase, re.I) and step_id.upper() != phase.upper():
                continue
        if (is_num_phase or is_cr_step) and step_name and (not step_name.startswith('---')):
            in_table = True
            rows.append({'step_id': step_id, 'step_name': step_name, 'criteria': criteria})
    return rows

def _extracted_block_127(path_parts: set[str], seg: str) -> None:
    path_parts.add(seg)
...
def _extract_indicators(criteria: str) -> list[str]:
    ...
    indicators: list[str] = []
    criteria = re.sub('^✅\\s*', '', criteria)
    for m in re.finditer('\\.eurika/[\\w/-]+', criteria):
        s = m.group(0).rstrip('/')
        if s not in indicators:
            indicators.append(s)
    for m in re.finditer('[\\w/]+\\.(?:py|md|json|yaml|yml)\\b', criteria):
        s = m.group(0)
        if s not in indicators:
            indicators.append(s)
    for m in re.finditer('\\b(README|CHANGELOG|ROADMAP|CYCLE_REPORT)\\b', criteria):
        s = m.group(1)
        if s not in indicators:
            indicators.append(s)
    ...
    for ind in indicators:
        ...
        for seg in ind.replace('.', '/').split('/'):
            if len(seg) > 2:
                _extracted_block_127(path_parts, seg)
    for m in re.finditer('\\b([A-Za-z_][A-Za-z0-9_]{2,})\\b', criteria):
        s = m.group(1)
...
def _verify_step_with_llm(step_id: str, step_name: str, criteria: str, code_snippets: str) -> tuple[bool, str | None, str | None]:
    ...
    if re.search('выполнено\\s*:', criteria_lower) or 'farm_helper' in criteria_lower or 'optweb' in criteria_lower or ('отработали штатно' in criteria_lower):
...
def _check_indicator(root: Path, indicator: str) -> bool:
    ...
    try:
        import subprocess
        r = subprocess.run(['grep', '-rl', '--include=*.py', indicator, str(root)], cwd=str(root), capture_output=True, text=True, timeout=5)
        return r.returncode == 0 and bool(r.stdout.strip())
    except Exception:
        pass
    return False

def _extracted_block_333(summary_parts: list[str], results: list) -> None:
...
def verify_phase(root: Path, phase: str, roadmap_path: Path | None=None, use_llm: bool=True) -> dict[str, Any]:
    """
    ...
    if not roadmap_path.exists():
        return {'ok': False, 'phase': phase, 'steps': [], 'summary': f'ROADMAP не найден: {roadmap_path}'}
    content = roadmap_path.read_text(encoding='utf-8')
    section = _find_phase_section(content, phase)
    if not section:
        return {'ok': False, 'phase': phase, 'steps': [], 'summary': f'Фаза {phase} не найдена в ROADMAP.'}
    ...
    for row in rows:
        ...
        if use_llm:
            ...
            llm_ok, llm_reason, llm_error = _verify_step_with_llm(row.get('step_id', ''), row.get('step_name', ''), criteria, code_snippets)
            if llm_reason is not None:
                step_ok = llm_ok
        if not step_ok:
            all_ok = False
        results.append({'step_id': row.get('step_id'), 'step_name': row.get('step_name'), 'found': found, 'missing': missing, 'ok': step_ok, 'llm_reason': llm_reason, 'llm_error': llm_error})
    ...
    if use_llm and (not any_llm):
        _extracted_block_333(summary_parts, results)
    return {'ok': all_ok, 'phase': phase, 'steps': results, 'summary': '\n'.join(summary_parts)}
...
def run_roadmap_verify(root: Path, message: str, use_llm: bool=True) -> tuple[str, dict[str, Any]]:
    ...
    if not phase:
        return ('Укажи фазу, например: «проверь фазу 2.7», «проверь фазу CR-B» или «verify phase 3.0».', {'ok': False})
