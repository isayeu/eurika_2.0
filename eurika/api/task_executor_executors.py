...
def _verify(root, smoke_path) -> Dict[str, Any]:
...
def _execute_ui_add_empty_tab(root: Path, spec: TaskSpec) -> ExecutionReport:
    ...
    if not target_file.exists():
        return ExecutionReport(ok=False, summary='ui tab change failed', error='target file not found: qt_app/ui/main_window.py')
    ...
    if add_line in src:
        verify = _verify(root, smoke_path)
        return ExecutionReport(ok=bool(verify.get('ok')), summary='tab already exists', skipped_steps=[f'insert {tab_name}'], verification=verify, artifacts_changed=[], error=None if verify.get('ok') else 'smoke failed')
    anchor = 'self.tabs.addTab(tab, "Chat")'
    pos = src.find(anchor)
    if pos < 0:
        return ExecutionReport(ok=False, summary='ui tab change failed', error='anchor not found: Chat tab')
    line_end = src.find('\n', pos)
    if line_end < 0:
        line_end = len(src)
    ...
    verify = _verify(root, smoke_path)
    return ExecutionReport(ok=bool(verify.get('ok')), summary=f'added tab `{tab_name}` after Chat', applied_steps=[f'insert {tab_name}'], verification=verify, artifacts_changed=['qt_app/ui/main_window.py'], error=None if verify.get('ok') else 'smoke failed')
...
def _execute_ui_remove_tab(root: Path, spec: TaskSpec) -> ExecutionReport:
    ...
    def _verify() -> Dict[str, Any]:
        if smoke_path.exists():
            return run_pytest(root, ['-q', 'tests/test_qt_smoke.py'], timeout=120)
        return {'ok': True, 'runner': 'pytest', 'output': 'smoke skipped (tests/test_qt_smoke.py not found)'}
    patt = re.compile(f"""^\\s*self\\.tabs\\.addTab\\([^,\\n]+,\\s*["\\']{re.escape(tab_name)}["\\']\\)\\s*$""", re.MULTILINE)
    updated, removed_count = patt.subn('', src)
    if removed_count == 0:
        verify = _verify()
        return ExecutionReport(ok=bool(verify.get('ok')), summary=f'tab `{tab_name}` already absent', skipped_steps=[f'remove tab `{tab_name}`'], verification=verify, artifacts_changed=[], error=None if verify.get('ok') else 'smoke failed')
    updated = re.sub('\\n{3,}', '\n\n', updated)
    task_backup_before_write(root, 'qt_app/ui/main_window.py', target_file)
    target_file.write_text(updated, encoding='utf-8')
    verify = _verify()
    return ExecutionReport(ok=bool(verify.get('ok')), summary=f'removed tab `{tab_name}`', applied_steps=[f'remove tab `{tab_name}`'], verification=verify, artifacts_changed=['qt_app/ui/main_window.py'], error=None if verify.get('ok') else 'smoke failed')
...
def _execute_delete(root: Path, spec: TaskSpec) -> ExecutionReport:
    ...
    return ExecutionReport(ok=ok, summary='deleted file' if ok else 'delete failed', applied_steps=['delete file'] if ok else [], verification={'runner': 'file_ops', 'ok': ok}, artifacts_changed=[msg] if ok else [], error=None if ok else msg)
...
def _execute_run_command(root: Path, spec: TaskSpec) -> ExecutionReport:
    ...
    out = ((res.stdout or '') + (res.stderr or '')).strip()
    return ExecutionReport(ok=res.returncode == 0, summary='command passed' if res.returncode == 0 else 'command failed', applied_steps=[f'run `{command}`'], verification={'runner': 'command', 'ok': res.returncode == 0, 'exit_code': res.returncode, 'output': out[:4000]}, artifacts_changed=[], error=None if res.returncode == 0 else f'exit {res.returncode}')