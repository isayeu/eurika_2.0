...
def _cosine_sim(a: List[float], b: List[float]) -> float:
    ...
    if na <= 0 or nb <= 0:
        return 0.0
    raw = dot / (na * nb)
    return max(0.0, min(1.0, (raw + 1) / 2))  # map [-1,1] → [0,1]
...
def _intent_exemplars(cfg: Dict[str, Any]) -> List[Tuple[str, str, Optional[str]]]:
    ...
    for handler_id, spec in intents.items():
        if not isinstance(spec, dict):
            continue
        emit = spec.get("emit")
        # CR-G2: vector_exemplars — curated phrases for fuzzy match (best for embeddings)
...
def match_fuzzy_intent(
    ...
    *,
    ...
    best_sim = threshold  # only matches with sim >= threshold can win
