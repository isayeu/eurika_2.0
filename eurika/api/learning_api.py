...
def get_pattern_library(project_root: Path, *, with_samples: bool=True, max_samples_per_kind: int=3) -> Dict[str, Any]:
    """
    ...
    for kind, entries in (lib or {}).items():
        ...
        for e in entries[:max_samples_per_kind if with_samples else 0]:
            if isinstance(e, dict):
                ...
                if with_samples:
                    samples.setdefault(kind, []).append({'project': proj, 'module': e.get('module'), 'location': e.get('location'), 'hint': (e.get('hint') or '')[:120]})
    return {'exists': True, 'counts': counts, 'projects': sorted(projects)[:10], 'samples': samples if with_samples else {}}
...
def _extracted_block_157(row: Dict[str, Any], rows: List[Dict[str, Any]]) -> None:
...
def _merged_stats_for_weak_pair(smell: str, action: str, by_smell_action) -> tuple[int, int]:
...
def get_learning_insights(project_root: Path, top_n: int=5, *, polygon_only: bool=False) -> Dict[str, Any]:
    ...
    for k, v in (by_smell_action or {}).items():
        ...
        if rate >= 0.3:
            parts = k.split('|', 2)
            prioritized_smell_actions.append({'smell_type': parts[0] if len(parts) > 0 else '?', 'action_kind': parts[1] if len(parts) > 1 else '?', 'verify_success_rate': round(rate, 4), 'total': total})
    ...
    for smell, action in WEAK_SMELL_ACTION_PAIRS:
        total, success = _merged_stats_for_weak_pair(smell, action, by_smell_action)
        if total < 5:
            continue
        rate = success / max(total, 1)
        if rate >= 0.25:
            policy_adjustment_hints.append({'pair': f'{smell}|{action}', 'verify_success_rate': round(rate, 4), 'total': total, 'hint': 'Consider moving deny→review in auto mode (REFACTOR_CODE_SMELL_PLAN Phase 4)'})
    return {'by_action_kind': by_action_kind, 'by_smell_action': by_smell_action, 'prioritized_smell_actions': prioritized_smell_actions[:10], 'by_target': ordered_targets[:max(top_n, 1) * 2], 'what_worked': ordered_targets[:top_n], 'recommendations': {'whitelist_candidates': whitelist_candidates, 'policy_deny_candidates': deny_candidates, 'policy_adjustment_hints': policy_adjustment_hints, 'chat_whitelist_hints': chat_recs.get('chat_whitelist_hints', []), 'chat_policy_review_hints': chat_recs.get('chat_policy_review_hints', [])}}