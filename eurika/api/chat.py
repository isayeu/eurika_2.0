...
def _run_eurika_fix(project_root: Path, dry_run: bool = False, timeout: int = 180) -> str:
    ...
    try:
        ...
        if dry_run:
            cmd.append("--dry-run")
        r = subprocess.run(
            [sys.executable, "-m", "eurika_cli", "fix", str(project_root), "--quiet"] + (["--dry-run"] if dry_run else []),
...
def _build_chat_prompt(
    ...
    if save_target:
        system = (
            "You are Eurika. The user asked you to write code and save it to a file. "
            "Generate ONLY the code. No questions, no apologies, no clarification requests. "
            "Output must contain a ```python code block."
        )
    else:
        system = (
            ...
            "When asked to write code, prefer Python; when asked about architecture, use the context."
        )
    ...
    if save_target:
        context_block += (
            ...
            "Reply ONLY with the code in a ```python block. Do NOT ask questions, do NOT apologize, do NOT request clarification. "
            "Generate the code immediately. Example format:\n```python\ndef foo(): ...\n```\n\n"
        )
...
def chat_send(
    ...
    # ROADMAP 3.5.11.C: refactor -> run eurika fix, skip LLM
    if intent == "refactor":
...
# TODO (eurika): refactor deep_nesting '_build_chat_context' — consider extracting nested block


# TODO (eurika): refactor long_function 'chat_send' — consider extracting helper
