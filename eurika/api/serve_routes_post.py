...
def dispatch_api_post(handler, project_root: Path, path: str, body: dict | None) -> bool:
    ...
    if path == '/api/exec':
        ...
        if 'timeout' not in body:
            ...
            if err:
                _serve._json_response(handler, {'error': err, 'hint': 'Expected timeout: integer, null, or 1..3600'}, status=400)
                return True
    ...
    if path == '/api/chat':
        ...
        if history is not None:
            if not isinstance(history, list):
                _serve._json_response(handler, {'error': 'invalid history payload', 'hint': 'Expected history: list of {role, content}'}, status=400)
                return True
            for i, item in enumerate(history):
                if not isinstance(item, dict):
                    _serve._json_response(handler, {'error': 'invalid history payload', 'hint': 'history items must be objects'}, status=400)
                    return True
                role = item.get('role')
                content = item.get('content')
                if not isinstance(role, str):
                    _serve._json_response(handler, {'error': 'invalid history payload', 'hint': 'history item role must be string'}, status=400)
                    return True
                if content is not None and not isinstance(content, str):
                    _serve._json_response(handler, {'error': 'invalid history payload', 'hint': 'history item content must be string'}, status=400)
                    return True
        from eurika.api.chat import chat_send
        result = chat_send(project_root, message, history=history)
        _serve._json_response(handler, result)
        return True
    if path == '/api/ask_architect':
        no_llm_raw = body.get('no_llm') if body else None
        no_llm = _parse_no_llm(no_llm_raw)
        if no_llm is None:
            _serve._json_response(handler, {'error': 'invalid no_llm payload', 'hint': 'Expected no_llm: boolean or "yes"/"true"/"1"'}, status=400)
            return True
