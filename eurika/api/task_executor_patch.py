...
def execute_code_edit_patch(root: Path, spec: TaskSpec) -> ExecutionReport:
    ...
    if occurrences != 1:
        return ExecutionReport(ok=False, summary='patch failed', error=f'old_text occurrences must be exactly 1, got {occurrences}')
    ...
    if is_dry_run:
        ...
        return ExecutionReport(ok=True, summary='patch dry-run preview', applied_steps=['compute replacement plan'], verification={'runner': 'dry_run', 'ok': True, 'verify_args': verify_args, 'output': f'{target}\n{preview}'}, artifacts_changed=[target])
    ...
    if verification.get('ok'):
        return ExecutionReport(ok=True, summary='patch applied and verified', applied_steps=['replace text', 'run verify'], verification=verification, artifacts_changed=[target])
    try:
        path.write_text(original, encoding='utf-8')
    except OSError as exc:
        return ExecutionReport(ok=False, summary='patch verify failed and rollback failed', applied_steps=['replace text', 'run verify'], verification=verification, artifacts_changed=[target], error=f'rollback failed: {exc}')
...
def execute_code_edit_patch_batch(root: Path, spec: TaskSpec, operations_json: str) -> ExecutionReport:
    ...
    for item in raw_ops:
        ...
        if len(old_text) > MAX_PATCH_TEXT or len(new_text) > MAX_PATCH_TEXT:
            return ExecutionReport(ok=False, summary='patch failed', error='text size exceeds limit')
        ops.append({'target': target, 'old_text': old_text, 'new_text': new_text, 'verify_target': str(item.get('verify_target') or '').strip()})
    ...
    for op in ops:
        ...
        if occurrences != 1:
            return ExecutionReport(ok=False, summary='patch failed', error=f'old_text occurrences must be exactly 1 in `{target}`, got {occurrences}')
    ...
    if is_dry_run:
        ...
        return ExecutionReport(ok=True, summary=f'batch patch dry-run preview ({len(ops)} ops)', applied_steps=[f'build {len(ops)} replacement plans'], verification={'runner': 'dry_run', 'ok': True, 'verify_args': verify_args, 'output': '\n'.join(preview_lines)}, artifacts_changed=changed_targets)
    written: List[Path] = []
    ...
    verification = run_pytest(root, verify_args, timeout=300)
    if verification.get('ok'):
        return ExecutionReport(ok=True, summary=f'batch patch applied and verified ({len(ops)} ops)', applied_steps=[f'apply {len(ops)} replacements', 'run verify'], verification=verification, artifacts_changed=changed_targets)
    ...
    if rollback_failed:
        return ExecutionReport(ok=False, summary='batch patch verify failed and rollback failed', applied_steps=[f'apply {len(ops)} replacements', 'run verify'], verification=verification, artifacts_changed=changed_targets, error='verify failed and rollback failed')
    verification['rollback'] = 'done'
    return ExecutionReport(ok=False, summary='batch patch verify failed, rollback done', applied_steps=[f'apply {len(ops)} replacements', 'run verify'], verification=verification, artifacts_changed=[], error='verify failed')