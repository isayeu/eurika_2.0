...
def format_capabilities_help() -> str:
    ...
    return """Да. В Qt у меня есть **локальный доступ к проекту**, вкладка **Terminal** и tool-loop к хосту.

**Что могу по системе (по нужде запроса):**
- факты о Linux-хосте через ```eurika-cmds```: `pwd`, сеть (`nmcli`/`ip`), логи (`journalctl`), сервисы (`systemctl`), устройства (в т.ч. CUPS/принтер)
- пакеты ОС (`pacman`/`apt`/…) — предложу и запущу; **sudo/пароль** спросит UI
- интернет/публичные API (цена тикера через Binance read-only, curl) и «поищи в интернете …»
- Cursor SDK / remote LLM — когда нужен свободный ответ или coding-agent
- **не** делаю тихий rewrite репозитория и live-ордера: правки кода → Approvals / «применяй»

**Как попросить код:**
- «напиши функцию … и сохрани в `app.py`»
- «создай файл `tests/test_foo.py` с тестами для …»
- «покажи файл …» → опиши правку → «сохрани» / «применяй»

**Проект (часто без LLM, мгновенно):**
- «что за проект?» — обзор и архитектура
- «сколько файлов?» / «пересчитай файлы» — подсчёт с диска
- «покажи дерево» / «структуру проекта» / «содержимое каталога» — список файлов
- «какие документы по проекту?» — README, docs/, правила
- «что дальше по развитию?» / «просмотри roadmap» — выжимка из ROADMAP/VISION
- «покажи файл app.py» — содержимое файла
- «выполни ls» — список в корне

**Анализ, Market, self-dev:**
- «просканируй проект» — `eurika scan .`
- «как успехи на маркете?» — paper-экзамен / learning report
- «посмотри стоимость BTC» — живой тикер
- «предложи полигон эксперимент» / bug-hunt — C.14 → Approvals (без auto-apply)
- «проведи ритуал» / smoke / self-check / release check
...
**Интернет (явная просьба или URL):**
- «поищи в интернете …» — веб-поиск (DuckDuckGo или Tavily/Brave из `.env`)

Спроси, например: «какой сейчас pwd?», «проверь cups», «стоимость ETH» или «что дальше по разработке?»."""
