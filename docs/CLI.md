...
| Propose (C.14 polygon HITL) | `eurika prove-cycle . --propose [--drill …]` | Seed polygon drill + pending plan **без** apply; `imports` / `extractable_block` / `long_function`; Approve → `--apply-approved` |
...
- `--drill` (только с `--propose`): `imports` (default), `extractable_block`, или `long_function`.
...
Chat: «предложи полигон эксперимент» / «второй полигон» / «третий полигон».
...
- `EURIKA_MAX_ACTIONS` — жёсткий лимит операций planner (RV7, review ~4505); default `20`; `0` — без лимита; effective cap = min(MAX_ACTIONS, MAX_OPS_PER_CYCLE) при обоих > 0
