...
| **eurika.reasoning.planner** | `analyze`, `detect_smells`, `propose_actions`, `Action`, `ActionPlan`, `ArchitectureModel`, `PatchOperation`, `PatchPlan`, `RefactorAction`, `RiskReport`, `SmellReport`, `SimulationResult`, `risk_report_from_plan`, `build_plan`, `build_action_plan`, `build_patch_plan` | `from eurika.reasoning.planner import analyze, propose_actions` |
...
| **eurika.analysis** | `graph`, `scanner`, `metrics`, `cycles`, `self_map`, `topology`, `scoring` | `from eurika.analysis import graph, scanner, scoring` |
