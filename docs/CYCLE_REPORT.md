...
- **144.** R10 Plugin system + Knowledge Graph — docs/archive/R10_EXTENSIBILITY_AND_KNOWLEDGE.md; plugins analyzer ✓; KG реализован (см. 150–155).
...
- **139. R6 Target v3 structure (2026-03):** docs/archive/TARGET_V3_STRUCTURE.md — world_model/reasoning/execution/memory; маппинг текущего кода; порядок миграции; приоритет стабилизации.
- **138. R7 Risks documented (2026-03):** docs/RISKS.md — 10 архитектурных рисков из review (Fragmented Intelligence, Patch Explosion, Memory Without Learning и др.) со статусом митигации. REVIEW_2026_IV_ANALYSIS R7 ✅.
- **137. R5 Learning central (2026-03):** Learning Loop формализован: Apply → record_outcome → [adapt_weights] → Plan reads get_merged_learning_stats. MEMORY.md §Learning Loop, Architecture.md §0.5.1 Learn stage.
- **136. R4 Chat API boundary (2026-03):** Документация в API_BOUNDARIES (Chat не импортирует patch_engine, patch_apply, orchestration; только subprocess). SubsystemBypassRule для eurika/api/chat → forbidden patch_engine, patch_apply, eurika.orchestration.
- **135. R2 reasoning: learning/feedback → eurika.storage (2026-03):** architecture_learning → eurika/storage/learning_store.py; architecture_feedback → eurika/storage/feedback_store.py. Root-level architecture_*.py — thin re-exports. persistence.py, event_views.py — импорт из eurika.storage. architecture_pipeline — отложен (core↔pipeline).
- **134. R2 reasoning consolidation: planner → facade (2026-03):** architecture_planner + architecture_planner_build_plan → eurika.reasoning.planner.facade. build_plan, build_action_plan, build_patch_plan в facade; architecture_planner и architecture_planner_build_plan — thin re-exports. Удалено SubsystemBypassException (circular import). REVIEW_2026_IV_ANALYSIS R2 частично.
- **133. R1 sdist hygiene (2026-03):** release_check.sh шаг 8b: build --sdist, проверка tar на __pycache__/.pyc. При наличии — FAIL. REVIEW_2026_IV_ANALYSIS R1 ✅.
- **132. Review IV фиксация (2026-03):** Новый docs/archive/review.md просмотрен. docs/archive/REVIEW_2026_IV_ANALYSIS.md — выводы, таблица задач (R1–R10), антипаттерны, связь с ROADMAP. ROADMAP §5.10 Review IV — задачи R1–R6, риски. Оценки: амбиция 9/10, модульность 7/10, AI 6/10. Приоритеты: единый reasoning engine, EnergyModel в центр, Chat API изолировать.
...
PlannerEngine вместо "God Engine": явные 4 шага collect_facts → generate_candidates → rank_candidates → output_plan (docs/archive/review.md §2).
