...
**Экспертное ревью (review.md, 2026):**
...
**Вывод ревью:** «Функционально мощный, архитектурно нестабильный, структурно перегруженный. Не готов к долгосрочному масштабированию без архитектурной стабилизации.» Риски: God CLI (разрастание core_handlers); backups в проекте; orchestration без формальной модели; сложность опережает архитектурную дисциплину.
...
### Эффективное использование Cursor AI

Поэтапные рекомендации по максимизации отдачи от Cursor AI. Нумерация **CR-A**, **CR-B**, … не пересекается с фазами ROADMAP (2.1, 3.0, 3.6.1 и т.д.). Порядок внедрения: CR-A1 → CR-A2 → CR-D1 → CR-C1 → CR-F1 → CR-B1 → CR-E1.

#### CR-A. Eurika Rules (.eurika/rules/)

**Цель:** зафиксировать контекст проекта, чтобы агент выдавал релевантные правки без повторных пояснений. Правила хранятся в **`.eurika/rules/`** (каталог Эврики). Cursor IDE может использовать симлинк `.cursor/rules` → `.eurika/rules` при необходимости.

| #    | Шаг                                         | Описание                                                                                 | Критерий готовности                                               |
| ---- | ------------------------------------------- | --------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| CR-A1 | Создать `.eurika/rules/eurika.mdc`           | Структура: CLI → orchestration → API → agent; ссылка на Architecture.md §0              | ✅ Файл существует; агент учитывает слои при ответе                |
| CR-A2 | Добавить venv и команды                     | Путь к venv: `.venv` (symlink); примеры: `eurika serve .`, `eurika scan .`, `pytest tests/ -q`  | ✅ venv.mdc; Commands tab — scan/doctor/fix в GUI (QProcess)        |
| CR-A3 | Зафиксировать dependency firewall            | L0–L6, no upward deps; EURIKA_STRICT_LAYER_FIREWALL=1 при проверке                      | ✅ Dashboard SELF-GUARD; кнопка «Показать детали»; get_firewall_violations_detail API |
| CR-A4 | Добавить Qt-контекст                         | PySide6, qt_app/, adapters/, thin shell поверх API; не трогать ядро Eurika               | ✅ qt_app.mdc: PySide6, adapters/ui/services, правила для агента   |


#### CR-B. Agent Skills

**Цель:** типовые операции оформлены как навыки — агент выполняет их стабильнее.

| #    | Шаг                                         | Описание                                                                                 | Критерий готовности                                               |
| ---- | ------------------------------------------- | --------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| CR-B1 | Skill «Тест для API endpoint»               | Шаблон: GET/POST handler → test в tests/test_api.py; проверка статуса и JSON            | ✅ .cursor/skills/api-endpoint-test/; тесты GET summary, self_guard (примеры) |
| CR-B2 | Skill «Release check»                        | Запуск release_check.sh; разбор типовых ошибок (ruff, mypy, pytest); исправления        | ✅ «прогони release check» запускает скрипт; вывод при падении сохраняется; «исправь» получает вывод в контексте; .eurika/skills/release-check/ |
| CR-B3 | Skill «Сверка ROADMAP с кодом»              | По фазе X.Y: найти реализацию, сравнить с критериями, обновить отметки ✅ в ROADMAP     | ✅ «проверь фазу 2.7» сверяет ROADMAP с кодом; roadmap_verify intent; .eurika/skills/roadmap-verify/ |


#### CR-C. Доработка стартового промпта

**Цель:** при старте чата агент сразу получает максимум полезного контекста.

| #    | Шаг                                         | Описание                                                                                 | Критерий готовности                                               |
| ---- | ------------------------------------------- | --------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| CR-C1 | Добавить ссылки на ключевые документы       | Architecture.md, DEPENDENCY_FIREWALL.md, CLI.md, API_BOUNDARIES.md                     | Промпт содержит список «Читай при необходимости: …»                |
| CR-C2 | Указать типовые команды проверки            | `eurika scan .`, `eurika doctor .`, `eurika fix . --dry-run`, `pytest tests/ -q`         | Промпт явно перечисляет команды для smoke-проверки                |
| CR-C3 | Добавить чек-лист перед коммитом             | Тесты, ruff, mypy (optional), release_check                                              | Агент перед коммитом предлагает пройти чек-лист                    |


#### CR-D. @-ссылки и контекст

**Цель:** целенаправленно подключать нужные файлы и папки в диалог.

| #    | Шаг                                         | Описание                                                                                 | Критерий готовности                                               |
| ---- | ------------------------------------------- | --------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| CR-D1 | Документировать рекомендованные @-ссылки    | @docs/ROADMAP.md — цели; @docs/Architecture.md — слои; @eurika/agent/ — agent runtime   | В ROADMAP или CONTRIBUTING есть раздел «Cursor: рекомендуемые @»   |
| CR-D2 | Паттерны по типам задач                     | Рефакторинг agent → @eurika/agent/; новый API → @eurika/api/; Qt → @qt_app/              | Разработчик знает, что подключать для типовой задачи              |
| CR-D3 | Исключения из контекста                      | .cursorignore для больших артефактов, build, __pycache__                                | Агент не загружает лишние файлы; ответы быстрее                   |


#### CR-E. Composer для крупных задач

**Цель:** сложные рефакторинги делать в Composer, а не в Chat.

| #    | Шаг                                         | Описание                                                                                 | Критерий готовности                                               |
| ---- | ------------------------------------------- | --------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| CR-E1 | Определить сценарии для Composer            | Разбиение монолитного модуля; добавление новой фазы в ROADMAP; вынос domain vs presentation | В ROADMAP или UI.md перечислены «Используй Composer для: …»       |
| CR-E2 | Шаблон промпта для Composer                 | Контекст + пошаговый план (3–7 шагов) + критерии приёмки на каждый шаг                  | Копируемый шаблон в ROADMAP или .cursor/                          |
| CR-E3 | Практика: один крупный рефакторинг в Composer | Выполнить реальный split/refactor через Composer, зафиксировать уроки                  | Есть пример в CYCLE_REPORT или CONTRIBUTING                        |


#### CR-F. Terminal и background tasks

**Цель:** агент умеет запускать нужные команды и интерпретировать вывод.

| #    | Шаг                                         | Описание                                                                                 | Критерий готовности                                               |
| ---- | ------------------------------------------- | --------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| CR-F1 | Документировать команды для агента          | `eurika serve .` (background), `pytest tests/ -q`, `scripts/release_check.sh`            | Список в ROADMAP или venv.mdc                                     |
| CR-F2 | Паттерн «изменение → проверка»              | После правок: pytest по затронутому модулю; при API — curl или тест                     | Агент по умолчанию запускает релевантную проверку                 |
| CR-F3 | Интерпретация типовых ошибок                | Ruff E402/F401 → добавить __all__ или импорт; mypy error → type hint; pytest fail → анализ | Агент распознаёт и предлагает исправления для частых ошибок      |

#### CR-G. Chat intents config (вынос команд из кода)

**Цель:** команды и паттерны чата хранятся в конфиге, не в коде. Фаза 1 — YAML; позже — векторная память (CR-G2).

| #    | Шаг                                         | Описание                                                                                 | Критерий готовности                                               |
| ---- | ------------------------------------------- | --------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| CR-G1 | Конфиг chat_intents.yaml                     | Паттерны, emit-команды, intent_hints в `.eurika/config/chat_intents.yaml`; loader + `match_direct_intent()` | ✅ Файл; eurika/api/chat_intents_config.py; chat использует config с fallback на legacy |
| CR-G2 | Векторная память (опционально)               | Embeddings для fuzzy match; Chroma/FAISS или Ollama embed                              | При низкой уверенности keyword → fallback на векторный поиск      |
| CR-G3 | PyTorch-классификатор (опционально)          | Только при 100+ интентов и большом объёме размеченных данных                             | —                                                                  |

**Структура config:** `intents.{handler_id}.patterns`, `exclude`, `exclude_prefix`, `require`, `emit`, `intent_hints`. Зависимость: `pyyaml` (extras).
...
#### Приоритет 0 — Экспертное ревью (review.md): немедленные действия

| # | Действие | Описание | Связь с ROADMAP |
|---|----------|----------|-----------------|
| P0.1 | Удалить `.eurika_backups` из проекта | Backups в рабочем дереве — критическая проблема: увеличивают размер в 2–3 раза, ломают поиск, риск импорта не той версии. Должно быть в Git history, не в дереве. | ✅ Очищено; release_check шаг 0 — предупреждение при наличии бэкапов |
| P0.2 | Вынести orchestration из CLI в application слой | CLI (core_handlers ~900 строк) знает слишком много о reasoning. Граница presentation/core нарушена. CLI выполняет роль application layer. | ✅ eurika/orchestration/ — application layer; cli/orchestrator — thin re-export; cli/orchestration — re-exports |
| P0.3 | Формальная модель pipeline | Input → Plan → Validate → Apply → Verify. Нет явной state machine — reasoning «магический чёрный ящик». | R2 Явная state-модель цикла |
| P0.4 | Ограничить размер файлов до 400 строк | Жёсткий бюджет в self-check/ритуале. | R1 Size budget (уже в плане) |
| P0.5 | Зафиксировать архитектурную диаграмму | Явная визуализация слоёв и потоков. | Architecture.md §0 |

**Доп. наблюдения ревью:** версионный хаос (eurika_2.0 / eurika_2.0.Qt / backups) — архитектурные решения копируются, не фиксируются. 58k строк требуют строгой модульности, покрытия тестами, CI, типизации, архитектурной документации.

**Главные риски (review):** 1) God CLI — разрастание в монолит; 2) копирование вместо рефакторинга (backups = отсутствие дисциплины); 3) orchestration без формальной модели — трудноуловимые баги; 4) сложность опережает контроль. **Честный ответ ревью:** «Строишь мощную систему, но больше добавляешь функционал, чем укрепляешь фундамент. Это главный стратегический риск Eurika.»
