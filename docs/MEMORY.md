...
**Paper bank 1000 USDT** — риск 1% equity на сделку (маржа); **soft → ×0.6 маржи** (`soft_sz×0.6`); futures lev 1…5× = **уверенность** (side prob), vol слегка гасит; суммарная маржа ≤30% equity; PnL USDT = edge×notional. **Комиссии:** spot `0.001` RT; futures `0.0008` RT. **Funding:** при закрытии futures — public Binance `fundingRate` (settlements в окне) или pro-rata `lastFundingRate` из `premiumIndex`; знаковый edge (`fund=…[history|premium_prorata]`). Cash-and-carry / фарм — отдельный режим.
...
**Paper bank (фаза 1):** `.eurika/ml/paper_portfolio.json` — старт **1000 USDT**, риск **1% equity** как маржа на сделку (**soft ×0.6**), futures lev **1…5×** от confidence/side_prob (spot=1); **soft futures ≤2×** (UTC 07–09 → 1×) + tighter soft SL/trail; суммарная маржа ≤ **30% equity**, отказ входа → journal `hold` «нет бюджета риска». Journal open/pending пишет `utc_hour` (наблюдение 08:00) и `soft_sz×…`. `pnl_usdt ≈ edge × notional` (убыток clamp к марже). Непрерывный банк (не сбрасывается при Live toggle). Фаза 2: ML-аллокация/плечо (частично), вес меток по `pnl_usdt` (entry ✅) / exit MFE-giveback ✅.
