...
**Формат ответа в Qt:** транскрипт рендерит лёгкий markdown — fenced-блоки `` ``` `` в рамке с **Copy**; для `bash`/`sh`/`shell`/`console` (и пустого lang с CLI-эвристикой) — ещё **Run** (запуск во вкладке Terminal через `execute_command_from_chat`). Clear сбрасывает payload-ссылки блоков.
