...
| опечатка вроде `scsn` | Подсказка «просканируй проект?» (не list_docs) |
...
| Фраза | Действие |
|-------|----------|
| `собери коммит`, `git status` | Статус, diff и предложение сообщения (по путям/diff, не «Update N files») |
