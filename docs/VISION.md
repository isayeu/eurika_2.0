...
| **3 Reasoner** | hypotheses · planning · decisions | planner, goals/reflection v1, chat interpreter; **Goal Model v0** в snapshot; **Hypothesis Engine v0** (`.eurika/hypotheses.json`, evidence+expected, CLI `hypotheses` / Chat «гипотезы»); **reject_pattern → bug-hunt ranking** (deprioritize, не deny); **multi-hypothesis ranking v0** (`rank_score`, multi reject kinds, confidence-scaled caution + prefer_safe); **planning coupling v0** (A/B bias + low `verify_by_kind` → soft score в bug-hunt) | planner-core coupling beyond bug-hunt |
| **4 Experimenter** | sandbox · A/B · rollback | polygon, prove-cycle, propose+sandbox, worktree; **Experiment Memory v0** (`.eurika/experiments.json`); **Formal A/B v0** (`.eurika/ab_trials.json`, baseline vs sandbox: energy/risk/smells/cycles/smoke_ok; CLI `ab-compare` / Chat «a/b»; hook после sandbox smoke; **не** autoapply); **A/B rescan auto** (`EURIKA_AB_RESCAN=auto|on|off`, default auto — rescan sandbox когда history metrics stable + self_map seeded) | richer benchmarks / multi-metric suites |
| **5 Self-improving** | controlled self-mod · anti-regression | C.14 HITL, idle self-dev, bug-hunt → Approvals; caps BOUNDED_EVOLUTION; Capability scores v0 + **HITL accept-rate** + **metrics beyond accept-rate** (apply_ok_rate, verify_by_kind, time_to_decide, hypotheses_supported → snapshot / hitl / experiments); Formal A/B v0 feeds experiment `metrics.ab_v0`; **planning coupling v0** читает A/B + verify_by_kind в ranking; A/B rescan-when-stable; **multi-hypothesis ranking v0** | planner-core coupling beyond bug-hunt |
