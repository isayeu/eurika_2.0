...
14. ~~**Саморазвитие через полигон (HITL)**~~ частично ✅ (ритуал v1) — `eurika prove-cycle . --propose [--drill imports|extractable_block|long_function|llm_extract] [--require-llm]` → Approvals → `eurika fix . --apply-approved`. `--require-llm` — live LLM без synthetic fallback (dogfood). Дальше: worktree sandbox. Детали: [ROADMAP.md](ROADMAP.md) §4.6, [POLYGON_VERIFY_PLAYBOOK.md](POLYGON_VERIFY_PLAYBOOK.md).
