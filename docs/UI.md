...
| Вкладка | Назначение |
|---------|------------|
| **Chat** (первая) | Chat-first: слева **рейка как в Cursor** — воркспейс = Project root, дети = чаты; **‹/›** сворачивает панель; **Новый чат** выбирает каталог; **+** — тред в этом root; ПКМ по чату — переименовать/удалить; ПКМ по воркспейсу — убрать из списка (каталог на диске не трогается). Подвкладки **Агент** / **Market**. Во время хода — в ленте под сообщением **Thinking** с шевроном и шагами (Read / Grepped / Running / Model), как в Cursor (CR-H5; Qt и Desktop). Правки агента → **Approvals**. Desktop Chat: один Send → `chat/send` (CR-H H4), без переключателя Eurika/Agent. Market: Live paper, без live-ордеров. |
| **Terminal** | Классический экран: ввод после `$ ` (Enter = Run), Stop/Clear сверху; вывод Commands + shell. Команды из Chat (`release_check`) стримятся сюда сразу, не после финального пузыря. |
