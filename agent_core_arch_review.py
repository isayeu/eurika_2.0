...
Read-only layer that:
- accepts InputEvent(type="arch_review") or type="arch_evolution_query",
- reads existing v0.1 artifacts (self_map, smells/summary, history, observations),
- returns structured "explain_risk" / "summarize_evolution" proposals inside Result.
"""
...
class ArchReviewAgentCore:
    ...
    def handle(self, event: InputEvent) -> Result:
        ...
        try:
            ...
            return Result(
                success=False,
                output={
                    "error": "arch_event_failed",
                    "message": str(exc),
                },
                side_effects=["ArchReviewAgentCore: exception during arch event"],
            )
        ...
        return Result(
            ...
            output={
                ...
                "expected": [
                    "arch_review",
                    "arch_evolution_query",
                    "arch_action_dry_run",
                    "arch_action_simulate",
                ],
                "actual": event.type,
            },
            side_effects=["ArchReviewAgentCore: unsupported event type"],
        )

    # Internal helpers -----------------------------------------------
    def _handle_arch_review(self, root: Path, window: int) -> Result:
        # 1. Load structural artifacts (self_map -> graph -> smells/summary)
        summary, smells, graph = self._load_structure(root)

        # 2. History-based evolution view
        history_info = self._load_history(root, window=window)

        # 3. Last observations snapshot (if present)
        observations_info = self._load_observations(root)

        # 4. Build DecisionProposals
        explain = self._build_explain_risk(summary, smells)
        evolution = self._build_summarize_evolution(
            trends=history_info["trends"],
            regressions=history_info["regressions"],
            evolution_report=history_info["evolution_report"],
        )
        ...
        return Result(
            ...
            side_effects=[
                "ArchReviewAgentCore: read self_map.json",
                "ArchReviewAgentCore: read architecture_history.json",
                "ArchReviewAgentCore: read eurika_observations.json",
            ],
        )
    ...
    def _handle_arch_action_dry_run(self, root: Path, window: int) -> Result:
        ...
        action_plan: ActionPlan = build_action_plan(
            project_root=str(root),
            summary=summary,
            smells=smells,
            history_info=history_info,
        ...
        proposal = DecisionProposal(
            ...
            arguments={"action_plan": action_plan.to_dict()},
            confidence=0.7,
            rationale=(
                "Action plan derived from architecture plan inputs: prioritized modules, "
                "architectural smells and history trends. Execution is not performed "
                "by this AgentCore and is expected to be handled by an external sandbox."
            ),
        )
    ...
    def _handle_arch_action_simulate(self, root: Path, window: int) -> Result:
        ...
        proposal = DecisionProposal(
            ...
            confidence=0.7,
            rationale=(
                "Simulation performed in a read-only sandbox based on the derived "
                "ActionPlan; no code changes were applied to the target project."
            ),
        )
        ...
        return Result(
            ...
            side_effects=[
                "ArchReviewAgentCore: read self_map.json",
                "ArchReviewAgentCore: read architecture_history.json",
                "ArchReviewAgentCore: sandbox dry-run executed",
            ],
        )

    def _load_structure(
        self, root: Path
    ) -> tuple[Dict[str, Any], List[ArchSmell], "ProjectGraph"]:
        ...
        load_self_map(self_map_path)  # ensure file is at least readable
    ...
    def _build_explain_risk(self, summary: Dict[str, Any], smells: List[ArchSmell]) -> DecisionProposal:
        ...
        if risks:
            rationale_lines.append(
                f"Top structural risks detected: {', '.join(risks[:3])}."
            )
        rationale_lines.append(
            "Risks are derived from import graph structure and architectural smell scores; "
            "no runtime behaviour is taken into account."
        )
    ...
    def _build_summarize_evolution(
        self,
        trends: Dict[str, str],
        ...
        rationale_lines: List[str] = [
            "Evolution trends are computed from the append-only architecture history "
            "over the last N snapshots (modules + dependencies + smells + centralization)."
        ]
        if regressions:
            rationale_lines.append(
                "Potential regressions were detected and highlighted for manual review."
            )
        else:
            rationale_lines.append(
                "No strong regressions detected in the observed window; focus can stay on guided evolution."
            )

        return DecisionProposal(
            action="summarize_evolution",
            arguments=arguments,
            confidence=0.85,
            rationale=" ".join(rationale_lines),
        )

    def _score_modules_by_smells(
        self,
        ...
        # Base scores by module from smells.
        for s in smells:
            for node in s.nodes:
                scores[node] = scores.get(node, 0.0) + float(s.severity)
                reasons.setdefault(node, []).append(
                    f"{s.type} (severity={s.severity:.2f})"
                )

        # Boost modules explicitly called out in summary risks.
        for risk in summary.get("risks", []):
            # risk format: "type @ file (severity=..)"
            if "@ " in risk:
                _, rest = risk.split("@ ", 1)
        ...
        # Optional: if smell trend is increasing, slightly boost all smelly modules.
        ...
        return {
            name: {"score": score, "reasons": reasons.get(name, [])}
    ...
    def _build_priority_list(
        ...
        return [
            ...
            for name, info in sorted(
                scored.items(), key=lambda kv: kv[1]["score"], reverse=True
            )
        ]
    ...
    def _build_prioritize_modules(
        ...
        rationale_lines = [
            "Module priorities are derived from accumulated architectural smell severities, "
            "explicit risks in the architecture summary and, optionally, smell trends from history."
        ]
    ...
    def _build_plan(
        ...
        """
        plan: ArchitecturePlan = build_plan(
            project_root=str(root),
            summary=summary,
            smells=smells,
            history_info=history_info,
        ...
        return DecisionProposal(
            action="suggest_refactor_plan",
            arguments=arguments,
            confidence=0.7,
            rationale=(
                "High-level refactor plan derived from prioritized modules, "
                "architectural smells and history trends. "
                "v0.3 draft uses a minimal plan representation; "
                "future versions will populate concrete steps."
            ),
        )
    ...
    def _build_action_plan(
        ...
        try:
            ...
            if not raw:
                raw = store.aggregate_by_action_kind()
            if raw:
                for key, d in raw.items():
                    total = d.get("total", 0)
                    success = d.get("success", 0)
                    learned_signals[key] = dict(
        ...
        plan: ActionPlan = build_action_plan(
            project_root=str(root),
            summary=summary,
            smells=smells,
            history_info=history_info,
        ...
        return DecisionProposal(
            ...
            confidence=0.7,
            rationale=(
                "Action plan derived from prioritized modules, architectural smells "
                "and history trends. Execution remains outside this AgentCore and "
                "is expected to be handled by an external sandbox."
                + (" Past success rates were used to nudge expected_benefit." if learned_signals else "")
            ),
        )
    ...
    def _build_patch_plan(
        ...
        return DecisionProposal(
            ...
            confidence=0.6,
            rationale=(
                "Patch plan derived from prioritized modules and their smells. "
                "Intended as a human-reviewable set of patch suggestions before "
                "any automated refactoring is attempted."
            ),
        )


# TODO: Refactor agent_core_arch_review.py according to architecture plan.
# Suggested steps:
...
# TODO: Refactor agent_core_arch_review.py (god_module -> refactor_module)
# Suggested steps:
