...
def test_learning_store_append_and_aggregate(tmp_path: Path) -> None:
    ...
    store.append(project_root=tmp_path, modules=['m1.py'], operations=[{'kind': 'refactor_module', 'target_file': 'm1.py', 'smell_type': 'god_module'}, {'kind': 'introduce_facade', 'target_file': 'm2.py', 'smell_type': 'bottleneck'}], risks=['god_module @ m1.py'], verify_success=True)
    store.append(project_root=tmp_path, modules=['m2.py'], operations=[{'kind': 'refactor_module', 'target_file': 'm2.py', 'smell_type': 'bottleneck'}], risks=['bottleneck @ m2.py'], verify_success=False)
