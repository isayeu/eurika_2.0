...
def test_executor_sandbox_dry_run(tmp_path: Path) -> None:
    plan = ActionPlan(actions=[Action(type='refactor_module', target='a.py', description='Split module', risk=0.3, expected_benefit=0.8)], priority=[0], total_risk=0.3, expected_gain=0.8)
...
def test_executor_sandbox_execute(tmp_path: Path) -> None:
    ...
    plan = ActionPlan(actions=[Action(type='refactor_module', target='b.py', description='Refactor b', risk=0.3, expected_benefit=0.8)], priority=[0], total_risk=0.3, expected_gain=0.8)
    sandbox = ExecutorSandbox(project_root=tmp_path)
