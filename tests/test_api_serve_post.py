"""Tests for eurika.api.serve POST handler validation (approve, exec, chat, ask_architect)."""
...
from eurika.api import serve_routes_post
...
def test_run_post_handler_exec_accepts_timeout_min_boundary(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(api_serve, "_json_response", _fake_json_response)
    monkeypatch.setattr(serve_routes_post, "exec_eurika_command", _fake_exec)
    handled = api_serve._run_post_handler(
        _DummyHandler(),
        tmp_path,
        "/api/exec",
