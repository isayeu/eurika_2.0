...
def _extracted_block_38(path, use_light):
    cfg = path / 'docs' / ('curated_repos.light.json' if use_light else 'curated_repos.example.json')
...
def handle_learn_github(args: Any) -> int:
    ...
    if search_query:
        ...
        if config_path is None:
            config_path = _extracted_block_38(path, use_light)
    ...
    if do_patterns or do_scan:
        ...
        projects = {str(project) for v in data.values() if isinstance(v, list) for e in v if isinstance(e, dict) for project in [e.get('project')] if project}
