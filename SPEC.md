...
## Текущий фокус (по review.md и ROADMAP.md)

Спецификация v0.1–v0.4 описывает достигнутый контракт. **Стратегический приоритет** — не расширение reasoning, а **операционный прорыв**: инструмент, который меняет код и отвечает за результат. Цель: полноценный цикл Scan → Diagnose → Plan → **Patch** → **Verify** (с пересканом и проверкой метрик) → **Rollback** при провале; явный Patch Engine (apply_patch, verify_patch, rollback_patch); три архитектурных автофикса; единый Event Engine; упрощение CLI до четырёх режимов (scan, doctor, fix, explain). Детали — в **review.md** и **ROADMAP.md** (блоки «Что по-прежнему не хватает», «План прорыва»).
