...
class MainWindow(QMainWindow):
    ...
    def _build_dashboard_tab(self) -> None:
        ...
        self.learning_widget_text.setPlaceholderText(
    ...
    def _build_models_tab(self) -> None:
        ...
        self.chat_timeout_spin.setRange(5, 300)
    ...
    def _build_chat_tab(self) -> None:
        ...
        self.chat_goal_view.setPlaceholderText(
    ...
    def _run_apply_approved(self) -> None:
        self._command_service.run_apply_approved(project_root=self.root_edit.text().strip())
    ...
    def _refresh_dashboard(self) -> None:
        ...
        if summary.get("error"):
            ...
            self.learning_widget_text.setPlainText(
                "No learning data yet. Run eurika scan/fix/cycle in selected project."
            )
            return
    ...
    def _render_approvals_table(self) -> None:
        ...
        for index, op in enumerate(self._pending_operations):
            ...
            self.approvals_table.setItem(index, 3, QTableWidgetItem(str(op.get("risk", ""))))
            combo = QComboBox()
    ...
    def _load_chat_preferences(self) -> None:
        ...
        try:
            ...
            timeout = 30
        self.chat_timeout_spin.setValue(min(300, max(5, timeout)))
    ...
    def _refresh_chat_goal_view(self) -> None:
        ...
        if isinstance(last_execution, dict) and last_execution:
            ...
            lines.append(
                f"- ok={last_execution.get('ok')}, verification_ok={last_execution.get('verification_ok')}, "
                f"summary={last_execution.get('summary', '-')}"
            )
        ...
        if has_pending_plan:
            ...
            if pending_target:
                self.chat_pending_label.setText(
    ...
    def _on_chat_result(self, payload: dict[str, Any]) -> None:
        ...
        # Best-effort second refresh in case dialog_state write is slightly delayed.
    ...
    def _activate_pending_controls_from_response(self, text: str) -> None:
        ...
        if not token:
            # Do not activate buttons from non-authoritative text confirmations.
            self._pending_plan_fallback_active = False
            return
    ...
    def _setup_ollama_health_timer(self) -> None:
        self._ollama_health_timer.setInterval(3000)
    ...
    def _start_ollama_server(self) -> None:
        ...
        self.ollama_output.append(
            "$ HSA_OVERRIDE_GFX_VERSION="
            f"{env.value('HSA_OVERRIDE_GFX_VERSION')} "
    ...
    def _refresh_ollama_models(self, user_initiated: bool = False) -> None:
        try:
            ...
            if user_initiated:
                self.ollama_output.append(
                    "[models] Ollama API недоступен. "
                    "Нажми `Start Ollama`, затем повтори `Refresh installed`."
                )
                self.ollama_install_status.setText("Installed: API unavailable")
            elif err_text != self._last_models_error:
                # Keep auto-refresh quiet; avoid flooding output when API is down.
                self.ollama_install_status.setText("Installed: API unavailable")
    ...
    def _install_selected_ollama_model(self) -> None:
        if self._ollama_task_process.state() != QProcess.NotRunning:
            ...
            return
        self._save_chat_preferences()
        model = self._resolve_ollama_model_to_install(
            self.ollama_custom_model_edit.text(),
            self.ollama_available_combo.currentText(),
        )
        if not model:
    ...
    def _on_ollama_task_stdout(self) -> None:
        chunk = bytes(self._ollama_task_process.readAllStandardOutput()).decode(
    ...
    def _on_ollama_task_stderr(self) -> None:
        chunk = bytes(self._ollama_task_process.readAllStandardError()).decode(
            "utf-8", errors="replace"
        )
        self._ollama_task_stdout += chunk
        for line in chunk.splitlines():
        ...
        if self._ollama_task_mode == "pull":
            if exit_code == 0:
                ...
                if self._ollama_task_model:
                    ...
                    if (
                        self.ollama_installed_combo.findText(self._ollama_task_model) >= 0
                    ):
