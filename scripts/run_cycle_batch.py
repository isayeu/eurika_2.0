...
def _print_report_snippet(project_root: Path, cycle: int) -> None:
    ...
    from eurika.storage.experience_store import get_recent_failures, get_kind_plan_failure_counts
    ...
    lines = ['', f'## CYCLE_REPORT @ cycle {cycle}', '| Метрика | Значение |', '|---------|---------|', f"| success_rate (overall) | {(success_rate if success_rate is not None else 'N/A')} |", f'| apply_rate (window) | {apply_rate} |', f'| rollback_rate | {rollback_rate} |', f'| top_failure_reason | {top_failure} |', f'| most_deprioritized_goal | {most_deprior} |', f'| most_successful_action_kind | {best_action} |', f'| events.json (KB) | {events_size:.1f} |', f'| pattern_library.json (KB) | {pl_size:.1f} |', '| **Был ли сдвиг поведения?** | (Yes/No + комментарий) |', '']
    text = '\n'.join(lines)
    print(text)
    try:
...
def _extracted_block_144():
    progress_file.write_text(json.dumps({'current_cycle': i, 'max_cycles': args.max_cycles, 'report_every': args.report_every}, indent=0))
...
def main() -> int:
    ...
    for i in range(1, args.max_cycles + 1):
        ...
        try:
            _extracted_block_144()
        except OSError:
            pass
        out = run_cycle(project, mode='fix', quiet=False, non_interactive=True, dry_run=args.dry_run, allow_low_risk_campaign=True)
        err = out.get('error')
        if err:
